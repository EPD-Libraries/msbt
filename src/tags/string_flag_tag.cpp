...
void StringFlagTag::Fill(u16 group_id, u16 type_id, tcb::span<const u8> data) {
...
void StringFlagTag::Fill(std::string_view group_name, TagParams params) {
...
std::string StringFlagTag::ToText() {
  return "<StringFlag Name='" + util::convert_to_utf8(m_flag_name) +
    "' Unk1='" + std::to_string(m_unknown_1) +
    "' Unk2='" + std::to_string(m_unknown_2) +
    "'/>";
}

void StringFlagTag::ToBinary(exio::BinaryWriter& writer) {
  writer.Write<u16>(2);
  writer.Write<u16>(1);
  writer.Write<u16>(
